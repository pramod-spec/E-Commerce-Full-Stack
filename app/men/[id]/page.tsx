"use client"; // 1. Client component banaya taaki direct onClick chal sake
...
export default function ProductPage({ params }: PageProps) {
  ...
  // unwrapping params using React's use() hook
  ...
  // 2. Client-side par single product data aur reviews fetch karna
  useEffect(() => {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || "Review add karne mein error aaya");
      }
    } catch (error) {
