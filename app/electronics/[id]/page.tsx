"use client"; // Client component taaki click handlers directly chal sakein
...
export default function ProductPage({ params }: PageProps) {
  ...
  // Next.js dynamic params ko client side unwrap karne ke liye React use() hook
  ...
  // Client-side api se single product data aur reviews fetch karna
  useEffect(() => {
  ...
  // Add To Cart aur Buy Now ke direct handlers
  ...
  // 💬 Submit Review/Comment Handler
  const handleCommentSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!commentText.trim()) return alert("Comment empty nahi chhod sakte!");
    ...
    try {
      ...
      if (res.ok) {
        // Naye review ko purani list me sabse top par append karna bina refresh ke
        setReviews([data.review, ...reviews]);
        setCommentText("");
        setCommentRating("5");
        alert(data.message || "Review successfully add ho gaya!");
      } else {
        alert(data.error || "Review submit karne me issue aaya.");
      }
    } catch (error) {
